...
class IMM(Generic[MT]):
    ...
    def mix_probabilities( #sverre: this is step 1 in the workflow of the IMM method.
        ...
        #sverre: mix_probabiliteis (my) are the probabilities that model s_k-1 was the true model
        #sverre: at time step k-1, given that model s_k is the true model at time step k (this is the posterior conditional)

        #sverre: predicted_mode_probabilities is the prior returned from the function (the one in the denominator)
        #sverre: it's given by Pr{s_k | z_1:k-1}
    ...
    def mix_states( #sverre: step 2: want to obtain a gaussian for every model s_k at time k, that is a best fit description of
        ...
        return mixed_states
        #sverre: mixed_states is list containing:
            #sverre: GaussParams (expected value and variance)

    def mode_matched_prediction( #sverre: bruk EKF til å predikere en ny tilstand for hvert filter/mode.
        self,
        mode_states: List[MT], #sverre: the list returned from mix_states()
        # The sampling time
        Ts: float,
    ) -> List[MT]:
        modestates_pred = [
            fs.predict(cs, Ts) for fs, cs in zip(self.filters, mode_states) #sverre: this step uses the EKF
        ]
    ...
    def mode_matched_update( #sverre: updates states for every measurement using EKF. Each component is a mode.
                                #oppdaterer prediksjonene til EKF med
        self,
        z: np.ndarray,
        immstate: MixtureParameters[MT],
    ...
    def update_mode_probabilities( #sverre: step 4. Oppdaterer sannsynligheten for en gitt mode (de forandres gjerne med målingene)
        self,
        z: np.ndarray,
        immstate: MixtureParameters[MT],
    ...
    def update( #steg 4, oppdater IMM
        self,
        z: np.ndarray,
        immstate: MixtureParameters[MT],
        sensor_state: Dict[str, Any] = None,
